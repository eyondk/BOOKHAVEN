...
class Forapproval extends Controller
{
    public function index()
    {
        ...
        $reservations = $rent->getWaitingApprovalReservationByUserID($user_id);
        ...
        if(isset($_POST['cancel_res'])) {
            if($rent->cancel_res($_POST)) {
                $data['message'] = "Cancelled successfully!";
                redirect('cancelled');
            }  else {
                $data['error_message'] = $user->errors;
            }
            //show($rent->cancel_res($_POST));
        }

        // show(['reservations' => $reservations]);

        $this->view('user/forapproval', ['reservations' => $reservations]);
    }
