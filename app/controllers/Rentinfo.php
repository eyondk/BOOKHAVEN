...
class Rentinfo extends Controller
{
    public function index()
    {
        ...
        $data['book_author'] = $bookData ? $bookData['B_AUTHOR'] : 'Book Author';
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            // print_r($formData);
            // echo '</pre>';
            $rent->validate_rent($formData);
            if (empty($rent->errors)) {
                $rent->insert_rent($formData);
                redirect('forapproval');
            } else {
                $data['error_message'] = $rent->errors;
            }
        }
        
        $data['errors'] =$rent->errors;
        $this->view('user/rentinfo', $data);
    }
